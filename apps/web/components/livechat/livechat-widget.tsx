...
import { getLivechatSettings } from '@/actions/livechat-settings'
...
export default function LiveChatWidget({ fingerprint }: LiveChatWidgetProps) {
    ...
    return (
        <>
            ...
            {/* Eye Catcher / Launcher */}
            {!isOpen && (
                <div
                    className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2"
                >
                    {showEyeCatcher && settings?.eyeCatcherEnabled && (
                        <div
                            className="bg-black text-white rounded-lg shadow-lg p-3 max-w-[200px] cursor-pointer animate-fade-in-up"
                            onClick={() => {
                                setShowEyeCatcher(false)
                                setIsOpen(true)
                            }}
                        >
                            <p className="text-sm font-medium">{settings?.eyeCatcherText || 'Need help?'}</p>
                            <button
                                className="absolute -top-2 -right-2 w-5 h-5 bg-gray-700 rounded-full flex items-center justify-center text-white text-xs border border-white"
                                onClick={(e) => {
                                    e.stopPropagation()
                                    setShowEyeCatcher(false)
                                }}
                    ...
                    <button
                        className="group relative w-14 h-14 flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 bg-black rounded-full shadow-2xl"
                        onClick={() => {
                            setIsOpen(true)
                            setShowEyeCatcher(false)
                        }}
